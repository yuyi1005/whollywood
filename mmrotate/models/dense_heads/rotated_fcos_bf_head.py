...
class RotatedFCOSBFHead(FCOSHead):
    ...
    def __init__(self,
                 ...
                 center_sample_rotated: bool = True,
                 max_distance_scale: float = 1.0, 
                 bbox_channel: int = 4,
                 additional_channel: int = 1,
                 ...
                 loss_cls: ConfigType = dict(
                     ...
                     alpha=0.25,
                     loss_weight=1.0),
                 loss_bbox_hbox: ConfigType = dict(
        ...
        self.bbox_channel = bbox_channel
        ...
        super().__init__(
            ...
            **kwargs)
        self.center_sample_rotated = center_sample_rotated
        self.max_distance_scale = max_distance_scale
        self.loss_bbox_hbox = MODELS.build(loss_bbox_hbox)
    ...
    def _init_layers(self):
        ...
        self.conv_reg = nn.Conv2d(
            self.feat_channels, self.bbox_channel, 3, padding=1)
        self.conv_angle = nn.Conv2d(
            self.feat_channels, self.additional_channel, 3, padding=1)
    ...
    def loss_by_feat(
        ...
        if len(pos_inds) > 0:
            ...
            pos_bbox_preds = torch.cat(
                [pos_bbox_preds, pos_angle_preds], dim=-1)
            ...
            loss_bbox_hbox = self.loss_bbox_hbox(
                distance2bbox(pos_points, pos_bbox_preds[..., :4]),
                distance2bbox(pos_points, pos_bbox_targets[..., :4]),
                weight=pos_centerness_targets,
                avg_factor=centerness_denorm)
            loss_bbox_tbox = self.loss_bbox_hbox(
                distance2bbox(pos_points, pos_bbox_preds[..., 4:8]),
                distance2bbox(pos_points, pos_bbox_targets[..., 4:8]),
                weight=pos_centerness_targets,
                avg_factor=centerness_denorm)
            if self.loss_angle is not None:
                # pos_angle_targets = self.angle_coder.encode(pos_angle_targets)
                loss_angle = self.loss_angle(
                    pos_bbox_preds[..., 8:], pos_bbox_targets[..., 8:],
            ...
            loss_bbox_hbox = pos_bbox_preds.sum()
            loss_bbox_tbox = pos_bbox_preds.sum()
        ...
        if self.loss_angle:
            return dict(
                loss_cls=loss_cls,
                loss_bbox=loss_bbox,
                loss_bbox_hbox=loss_bbox_hbox,
                loss_bbox_tbox=loss_bbox_tbox,
    ...
    def get_targets(
        ...
        for i in range(num_levels):
            ...
            if self.norm_on_bbox:
                bbox_targets[..., :8] = bbox_targets[..., :8] / self.strides[i]
    ...
    def _get_targets_single(
        ...
        # top = h / 2 + offset_y
        # bottom = h / 2 - offset_y
        # rotated_target = torch.stack((left, top, right, bottom), -1)
        
        bbox_targets = self.bbox_coder.encode(points, gt_bboxes)
        ...
        max_regress_distance *= self.max_distance_scale
    ...
    def _predict_by_feat_single(self,
        ...
        # priors = torch.tensor([[100, 100], [100, 100], [100, 100], [100, 100], [100, 100], [100, 100]])
        # bboxes = torch.tensor([[100, 100, 64, 32, 0.78], [100, 100, 64, 32, 0.79], [100, 100, 64, 32, -0.01], [100, 100, 64, 32, 0.2], [100, 100, 64, 32, 0.4], [100, 100, 32, 64, 0.3]])
        # bboxes = self.bbox_coder.encode(priors, bboxes)
        # print(bboxes)
        # bboxes = self.bbox_coder.decode(priors, bboxes, max_shape=img_shape)
        # print(bboxes)
        # aaa
