...
from mmrotate.structures import RotatedBoxes, rbox2qbox
...
class FCOSHDRBoxHead(FCOSHead):
    ...
    def _init_cls_real_convs(self) -> None:
        """Initialize classification conv layers of the head."""
        self.cls_real_convs = nn.ModuleList()
    ...
    def _init_layers(self):
        ...
        self._init_cls_real_convs()
        self.conv_real_cls = nn.Conv2d(
        ...
        # self.ln0 = nn.LayerNorm([256, 128, 128])
        # self.ln1 = nn.LayerNorm([256, 128, 128])
        # self.ln2 = nn.LayerNorm([256, 128, 128])
        # self.ln3 = nn.LayerNorm([256, 128, 128])
        # self.ln4 = nn.LayerNorm([256, 128, 128])
    ...
    def forward(
        ...
        scale_preds = torch.pow(2, phase / (2 * torch.pi / self.num_step) - 1)  # Map to [1, 32)

        cls_feat = KV
        cls_real_feat = KV.detach()
        reg_feat = KV
        ...
        for cls_real_layer in self.cls_real_convs:
            cls_real_feat = cls_real_layer(cls_real_feat)
        cls_real_score = self.conv_real_cls(cls_real_feat)
        ...
        return (cls_score,), (bbox_pred,), (angle_pred,), (centerness,), K, (cls_real_score,)
    ...
    def loss_by_feat(
        ...
        flatten_cls_real_scores = [
            cls_real_score.permute(0, 2, 3, 1).reshape(-1, self.cls_out_channels)
            for cls_real_score in cls_real_scores
        ]
        flatten_bbox_preds = [
        ...
        flatten_cls_real_scores = torch.cat(flatten_cls_real_scores)
        ...
        cls_real_weight = ((flatten_labels < 0)
                    | (flatten_labels >= bg_class_ind)
                    | (flatten_angle_targets[..., 0] == -100)).float()
        pos_point_inds = ((flatten_labels >= 0)
                    & (flatten_labels < bg_class_ind)
                    & (flatten_angle_targets[..., 0] == -100)).nonzero().reshape(-1)
        num_point_pos = torch.tensor(
            len(pos_point_inds), dtype=torch.float, device=bbox_preds[0].device)
        num_point_pos = max(reduce_mean(num_point_pos), 1.0)
        loss_cls = self.loss_cls(
            flatten_cls_scores, flatten_labels, avg_factor=num_point_pos)
        loss_cls_real = self.loss_cls(
            flatten_cls_real_scores, flatten_labels, weight=cls_real_weight, avg_factor=num_point_pos)
    ...
    def _get_targets_single(
        ...
        # Recognize point annotations from dummy value 48 in size
        gt_point = torch.logical_and(gt_wh[..., 0] == 48, gt_wh[..., 1] == 48)
    ...
    def predict(self,
        ...
        predictions = self.predict_by_feat(
            *outs, batch_img_metas=unpack_gt_instances(batch_data_samples), rescale=rescale)
        return predictions
    ...
    def predict_by_feat(self,
                        ...
                        K: List[Tensor] = None,
                        cls_real_scores: List[Tensor] = None,
        ...
        for img_id in range(len(batch_img_metas[2])):
            img_meta = (batch_img_metas[0][img_id], batch_img_metas[1][img_id], batch_img_metas[2][img_id])
    ...
    def _predict_by_feat_single(self,
                                ...
                                cfg: ConfigDict,
                                rescale: bool = False,
                                with_nms: bool = True, K=None) -> InstanceData:
        ...
        gt_instances = img_meta[0]
        img_meta = img_meta[2]
        img_shape = img_meta['img_shape']
        gt_bboxes = gt_instances.bboxes
        gt_labels = gt_instances.labels
        gt_pos = (gt_bboxes[:, 0:2] / 8).long()
        gt_idx = gt_pos[:, 1] * 128 + gt_pos[:, 0]

        bbox_pred, angle_pred = bbox_pred_list[0], angle_pred_list[0]
        ...
        results.scores = gt_labels * 0 + 1
        results.labels = gt_labels
        self.save_dota_format(results, img_meta)
        ...
        for level_idx, (
            ...
            results = filter_scores_and_topk(
                ...
                dict(
                    bbox_pred=bbox_pred, angle_pred=angle_pred, priors=priors, yy=K))
            scores, labels, keep_idxs, filtered_results = results

            bbox_pred = filtered_results['bbox_pred']
            angle_pred = filtered_results['angle_pred']
            priors = filtered_results['priors']
            K = filtered_results['yy']
            ...
            # self.coef_sin = self.coef_sin.to(yy)
            # self.coef_cos = self.coef_cos.to(yy)
            # phase_sin = torch.sum(yy * self.coef_sin, dim=-1)
            # phase_cos = torch.sum(yy * self.coef_cos, dim=-1)
            # fp_pred = torch.atan2(phase_sin, phase_cos)  # In range (-pi,pi]
            # fp_pred[fp_pred < 0] += torch.pi * 2
            # fp_pred /= torch.pi * 2  # In range [0, 1)
            # fp_pred = (torch.pow(2, fp_pred) - 1) * (512 - 32) + 32  # Map to [32, 512)
            # mlvl_labels.append(fp_pred.long())

            # y2 = torch.floor(yy * 10).long()
            # y3 = y2[:, 0] * 10000 + y2[:, 1] * 1000 + y2[:, 2] * 100 + y2[:, 3] * 10 + y2[:, 4]
            # mlvl_labels.append(y3)
    ...
    def save_dota_format(self, results, img_meta):
        ...
        with open('pseudo_labels/' + img_meta['img_id'] + '.txt', 'w') as f:
