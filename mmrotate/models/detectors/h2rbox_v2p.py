    ...
    def loss(self, batch_inputs: Tensor,
        ...
        if torch.rand(1) < 0.95:
            batch_inputs_all = torch.cat(
                (batch_inputs, batch_inputs_rot))
            batch_data_samples_all = []
            for gt_instances in batch_gt_instances + batch_gt_rot:
                data_sample = DetDataSample()
                data_sample.gt_instances = gt_instances
                batch_data_samples_all.append(data_sample)
        else:
            batch_inputs_all = torch.cat(
                (batch_inputs, batch_inputs_flp))
            batch_data_samples_all = []
            for gt_instances in batch_gt_instances + batch_gt_flp:
                data_sample = DetDataSample()
                data_sample.gt_instances = gt_instances
                batch_data_samples_all.append(data_sample)
